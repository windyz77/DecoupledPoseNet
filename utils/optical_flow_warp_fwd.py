    ...
    def _transform(flo, input_dim, out_size):
        with tf.variable_scope('_transform'):
            ...
            input_transformed = _interpolate(input_dim, x_t_flat, y_t_flat,
                                             out_size)
            # num_batch = list(map(float,num_batch))
            # num_channels=list(map(float,num_channels))
            output = tf.reshape(
                input_transformed,
                tf.stack([num_batch, tf.to_int32(out_height), tf.to_int32(out_width), num_channels]))
            return output
