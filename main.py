...
from utils.loss_utils import average_gradients
...
# flags.DEFINE_string('trace', "../odometry_test/depthflow_10_28_ttrr_ttrr", 'directory for model checkpoints.')
# flags.DEFINE_string('trace', "../odometry_test/depthflow_11_2_ttrr_ttrr", 'directory for model checkpoints.')
# flags.DEFINE_string('trace', "../odometry_test/depthflow_11_7_ttrr_ttrr", 'directory for model checkpoints.')
flags.DEFINE_string('trace', "../odometry_test/depthflow_without_01", 'directory for model checkpoints.')
flags.DEFINE_integer('num_iterations', 300000,
                     'number of training iterations.')
flags.DEFINE_string('pretrained_model', '/home/yzm/BestModelUnPoseNet/model-30003',
                    'filepath of a pretrained model to initialize from.')
flags.DEFINE_string(
    'mode', 'depthflow',
    'selection from four modes of ["flow", "depth", "depthflow", "stereo"]')
flags.DEFINE_string('train_test', 'test', 'whether to train or test')
flags.DEFINE_boolean("retrain", False, "whether to reset the iteration counter")

flags.DEFINE_string('data_dir', '/home/yzm/raw_data/data_eigen/', 'root filepath of data.')
flags.DEFINE_string('train_file',
                    './filenames/kitti_train_files_png_4frames.txt',
                    'training file')
flags.DEFINE_string('gt_2012_dir', '/home/yzm/raw_data/kitti_2012/training',
                    'directory of ground truth of kitti 2012')
flags.DEFINE_string('gt_2015_dir', '/home/yzm/raw_data/kitti_2015/training',
...
flags.DEFINE_boolean("eval_flow", False, "whether to reset the iteration counter")
flags.DEFINE_boolean("eval_depth", False, "whether to reset the iteration counter")
flags.DEFINE_boolean("eval_mask", False, "whether to reset the iteration counter")

flags.DEFINE_integer('num_scales', 4, 'batch size for training')
...
flags.DEFINE_string('eval_pose', '09,10', 'pose seq to evaluate')
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
                    ...
                    with tf.name_scope(scopename) as ns:
                        if i == 0:
                            ...
                            var_pose = list(
                                set(
                                    tf.get_collection(
                                        ...
                                        scope=".*pose_net.*")))
                            var_pose_refine = list(
                                set(
                                    tf.get_collection(
                                        tf.GraphKeys.TRAINABLE_VARIABLES,
                                        scope=".*pose_net_refine.*")))
                            var_depth = list(
                                set(
                                    tf.get_collection(
                                        tf.GraphKeys.TRAINABLE_VARIABLES,
                            ...
                            if FLAGS.mode == "depthflow":
                                var_train_list = var_pose + var_depth + var_flow + var_pose_refine
                            elif FLAGS.mode == "depth":
                                var_train_list = var_pose + var_depth + var_pose_refine
        ...
        if FLAGS.pretrained_model:
            if FLAGS.train_test == "test" or (not FLAGS.retrain):
                ...
                saver_stereo = tf.train.Saver(
                    tf.get_collection(
                        tf.GraphKeys.MODEL_VARIABLES,
                        scope=".*(depth_net|feature_net_disp).*"),
                    max_to_keep=1)
                saver_stereo.restore(sess, FLAGS.pretrained_model2)
            else:
                raise Exception(
